<?php 
    ...
    $bandejaSalida = $mensajes->cargarMensajesEnviados($_SESSION['codigoUsuario']);

    $datosMensaje;
    $arrayDatos = array();
    
    for ($i=0; $i < count($bandejaSalida); $i++) { 
        

         $datosMensaje = $mensajes->datosMensaje($bandejaSalida[$i]['receptor'] );
...
<body>
    ...
    <?php
        ...
        $html .= '<div class="list-content"><ul class="list-group"><li href="#" class="list-group-item title">Mensajes enviados</li>';
                           
				for ($i = 0; $i < count($bandejaSalida); $i++) {
					$html .= '<li href="#" class="list-group-item text-left mensajeMostrado">'; 
                    
                        $html .= "<div class = 'barraMostrada panel'> ";
                        
                             $html .= "<span class = 'glyphicon glyphicon-plus pull-right mostrarCaja'> </span><div  class='asunto'> <b>Destinatario</b>: " . $arrayDatos[$i]['nombre'] .' ' .$arrayDatos[$i]['apellido'] . "</div>";
                    
                        $html .= "</div>";
                
                    $html .= '<div class = "cajaMostrada cajaMostradaOculta">'; 
                    
                        $html .= "<div  class='asunto'  > <span class = 'text-success'>ASUNTO:</span> " . $bandejaSalida[$i]['asunto'] . "</div><br>";
                        $html .= "<div  class='asunto'  > <span class = 'text-success'>MENSAJE</b></span>";
                        $html .= "<div class='mensaje'>" . $bandejaSalida[$i]['mensaje'] . "</div>";
                        $html .= 	'<br><a class="btn btn-danger btn-xs glyphicon glyphicon-trash eliminarMensaje" href = "../amigos/eliminarMensajesEnviados.php?idMensajeaBorrar= ' . $bandejaSalida[$i]['idMensaje'] . ' ""></a>';
